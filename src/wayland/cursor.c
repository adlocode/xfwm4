...
static void process_cursor_move(xfwmServer *server) {
...
static void process_cursor_resize(xfwmServer *server) {
	View *view = server->grabbed_view;
...
static void process_cursor_motion(xfwmServer *server, uint32_t time) {
...
static void handle_cursor_button(struct wl_listener *listener, void *data) {
	...
	double sx, sy;
	struct wlr_surface *surface = NULL;
	View *view = get_view_at(cursor->server,
