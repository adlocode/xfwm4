...
static struct wlr_scene_tree *wb_layer_get_scene(Output *output,
...
static xfwmLayerSurface *wb_layer_surface_create(
...
static void handle_unmap(struct wl_listener *listener, void *data) {
	...
	View *view = wl_container_of(surface->server->views.next, view, link);
	if (view && view->scene_tree && view->scene_tree->node.enabled) {
		focus_view(view, view->xdg_toplevel->base->surface);
	}
}

static void wb_layer_surface_destroy(xfwmLayerSurface *surface) {
	if (surface == NULL) {
		return;
	}
...
static xfwmLayerSurface *popup_get_layer(
		xfwmLayerPopup *popup) {
...
static void handle_new_popup(struct wl_listener *listener, void *data) {
	xfwmLayerSurface *wb_layer_surface =
