...
get_active_output (Client *c)
{
    ...
    wlr_output_layout_closest_point(c->server->output_layout, output,
            c->geometry.x + c->geometry.width / 2,
            c->geometry.y + c->geometry.height / 2,
            &closest_x, &closest_y);
...
xdg_toplevel_map (struct wl_listener *listener, void *data)
{
    ...
    struct wlr_box usable_area = get_usable_area(c);
    wlr_xdg_surface_get_geometry(c->xdg_toplevel->base, &geo_box);
    
        c->geometry.height = MIN(geo_box.height, usable_area.height);
        c->geometry.width = MIN(geo_box.width, usable_area.width);
        c->geometry.x = 0;
        c->geometry.y = 0;	
...
xdg_toplevel_request_maximize (struct wl_listener *listener, void *data)
{
    ...
    if (!is_maximized) {		
        c->previous_geometry = c->geometry;
        
            c->geometry.x = 0;
            c->geometry.y = 0;		
    } else {
        usable_area = c->previous_geometry;
        c->geometry.x = c->previous_geometry.x;
        c->geometry.y = c->previous_geometry.y;
    }
...
xdg_toplevel_request_minimize (struct wl_listener *listener, void *data)
{
    ...
    if (minimize_requested) {
        ...
        Client *next_view = wl_container_of(c->link.next, next_view, link);
        if (wl_list_length(&c->link) > 1)
            focus_view(next_view, next_view->xdg_toplevel->base->surface);
        else
            focus_view(c, c->xdg_toplevel->base->surface);
    } else {
        c->geometry = c->previous_geometry;
    }
...
begin_interactive (Client *c, enum CursorMode mode, uint32_t edges)
{
    ...
    if (mode == XFWM_CURSOR_MOVE) {
        server->grab_x = server->cursor->cursor->x - c->geometry.x;
        server->grab_y = server->cursor->cursor->y - c->geometry.y;
    } else if (mode == XFWM_CURSOR_RESIZE) {
        struct wlr_box geo_box;
        wlr_xdg_surface_get_geometry(c->xdg_toplevel->base, &geo_box);

        double border_x = (c->geometry.x + geo_box.x) +
            ((edges & WLR_EDGE_RIGHT) ? geo_box.width : 0);
        double border_y = (c->geometry.y + geo_box.y) +
        ...
        server->grab_geo_box.x += c->geometry.x;
        server->grab_geo_box.y += c->geometry.y;
...
handle_new_popup (struct wl_listener *listener, void *data)
{
    ...
    struct wlr_output *wlr_output = wlr_output_layout_output_at(
            ...
            c->geometry.x + popup->current.geometry.x,
            c->geometry.y + popup->current.geometry.y);
    ...
    struct wlr_box output_toplevel_box = {
        .x = output->geometry.x - c->geometry.x,
        .y = output->geometry.y - c->geometry.y,
...
handle_new_xdg_surface (struct wl_listener *listener, void *data)
{
    ...
    if (xdg_surface->role == WLR_XDG_SURFACE_ROLE_TOPLEVEL)
    {
        ...
        struct wlr_xdg_toplevel *toplevel = c->xdg_toplevel;
